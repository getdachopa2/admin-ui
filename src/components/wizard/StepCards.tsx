        ...
        <div className="space-y-3 max-h-72 overflow-auto pr-1">
          {manualCards.map((c, idx)=> (
            <div key={idx} className="grid grid-cols-1 items-end gap-2 sm:grid-cols-5">
              <Clearable label="CC No" value={c.pan}   onChange={(v)=> updateCard(idx,'pan',v)}   placeholder="4111 1111 1111 1111" />
              <Clearable label="Ay"    value={c.month} onChange={(v)=> updateCard(idx,'month',v)} placeholder="MM" />
              <Clearable label="Yıl"   value={c.year}  onChange={(v)=> updateCard(idx,'year',v)}  placeholder="YY veya YYYY" />
              <Clearable label="CVV"   value={c.cvv}   onChange={(v)=> updateCard(idx,'cvv',v)}   placeholder="3-4 hane" />
              <div className="flex items-end gap-2">
                <Clearable label="Banka" value={c.bank || ''} onChange={(v)=> updateCard(idx,'bank',v)} placeholder="örn. 62" />
